...
def know_page():
    ...
    with col2:
        ...
        if st.button("保存Q&A到知识库"):
            if 'qa_pairs' in st.session_state and pdf_file is not None:
                # 保存Q&A文本
                upload_txt_to_knowledge_base(st.session_state['qa_pairs'], pdf_file.name.replace(".pdf", "") + "5-qa.txt")

                # 保存PDF文件
                upload_pdf_response = upload_pdf_to_knowledge_base(pdf_file)

                # 根据上传结果显示信息
                if upload_pdf_response.status_code == 200:
                    st.success("PDF和Q&A对已保存到知识库！")
                else:
                    st.error("上传PDF失败：" + str(upload_pdf_response.get("message", "")))
